<script lang="ts">
	...
	let { onShowModelSelector }: Props = $props();
...
<ScrollArea class="h-[100vh]">
	<Sidebar.Header class=" top-0 z-10 gap-6 bg-sidebar/50 px-4 py-4 pb-2 backdrop-blur-lg md:sticky">
		...
		<ChatSidebarActions {handleMobileSidebarItemClick} bind:isSearchModeActive bind:searchQuery {onShowModelSelector} />
	</Sidebar.Header>

	<Sidebar.Group class="mt-4 space-y-2 p-0 px-4">
		{#if (filteredConversations.length > 0 && isSearchModeActive) || !isSearchModeActive}
			<Sidebar.GroupLabel>
				{isSearchModeActive ? 'Sökresultat' : 'Konversationer'}
			</Sidebar.GroupLabel>
		{/if}
...
<DialogConfirmation
	...
	description={selectedConversation
		? `Är du säker på att du vill radera "${selectedConversationNamePreview}"? Det går inte att ångra och alla meddelanden i konversationen tas bort permanent.`
		: ''}
	confirmText="Radera"
	cancelText="Avbryt"
	variant="destructive"
...
<AlertDialog.Root bind:open={showEditDialog}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title>Redigera konversationsnamn</AlertDialog.Title>
			<AlertDialog.Description>
				<Input
					...
					placeholder="Ange ett nytt namn"
					type="text"
		...
		<AlertDialog.Footer>
			...
			>
			<AlertDialog.Action onclick={handleConfirmEdit}>Spara</AlertDialog.Action>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>
