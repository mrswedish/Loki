<script lang="ts">
	...
	// Tauri: track whether llama-server is running
	let tauriServerReady = $state(!isTauriEnv()); // non-Tauri is always "ready"
	let showModelPicker = $state(false);

	// Initialize Tauri server URL on mount (async)
	$effect(() => {
		if (!isTauriEnv()) return;
		fetchServerUrl().then((url) => {
			if (url) tauriServerReady = true;
		});
	});
	...
	// Initialize server properties on app load (run once, wait for Tauri server)
	$effect(() => {
		// In Tauri mode, wait until the server URL is known
		if (!tauriServerReady) return;
	...
	$effect(() => {
		...
		if (
			...
			fetch(`${getServerBase() || base}/props`, { headers })
				.then((response) => {
